...
`no-repeated-signal-in-conditional.ts` → `noRepeatedSignalInConditional`. If a rule has multiple message IDs, prefix
each with the camelCase rule name (e.g. `noRepeatedSignalInConditionalFix`).
